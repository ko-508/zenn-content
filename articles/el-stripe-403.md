...
Stripe では API キーに対して細かい権限制御（Restricted API Keys）が可能です。制限付きキーを使用している場合、必要な操作に対応する権限が付与されていないと 403 が発生します。
...
テスト用 API キー（`sk_test_`）で本番環境のリソースにアクセスしたり、その逆を行おうとすると 403 が返されます。Stripe は環境を厳密に分離しているため、キーとリソースの環境が一致していないとアクセスが拒否されるのです。
...
Stripe は冪等性キー（`Idempotency-Key`）を使った安全な再試行をサポートしていますが、特定の制限付きキーではこの機能が使用禁止になる場合があります。
