...
このコードは自動的に500エラーで最大3回までリトライしており、各リトライ間隔は2秒、4秒、8秒となります。
...
status.slack.com で障害が報告されていない場合、まずリクエストのタイムアウト設定を確認してください。ネットワーク遅延でタイムアウトになり、結果として500エラーに見えることがあります。
...
また、Bot Tokenの権限不足やリクエストペイロードの不正も稀に500エラーを引き起こします。Slack APIドキュメントでそのメソッドに必要な権限スコープを確認し、Workspace設定の「OAuth & Permissions」タブで権限が付与されているか検確してください。それでも解決しない場合は、シンプルなテストリクエスト（例：auth.test メソッド）を実行して、APIの基本的な接続性が確保されているか確認します。
