...
なお、古い形式の応答では `details` が付かず、`message` の文言に指標・上限・サービス・利用者が書かれているだけの場合もあります。読む場所が変わるだけで、読むべき内容は同じです。

## まず最初に：QuotaFailure を開く

第一に、`details` の中に上限に関する詳細があるかを見ます。あれば、そこに答えが書かれています。無ければ、`message` の文言から同じ情報を読み取ります。
...
### 原因3：待つべき時間を無視している、または待っても回復しない型の上限
...
for i in range(5):
    ...
    time.sleep(delay if delay else 30 * (2 ** i))
```

指示が付いていない場合の公式の指針は、指数的に間隔を伸ばし、ランダム性を加える方式です。429 のクォータのエラーについては、最初の間隔を最短でも30秒とするよう公式文書に書かれています（500 や 503 のサーバー側のエラーは最短1秒で、扱いが違います）。一定の間隔で叩き続けると、上限を使い切った状態から抜け出せません。

もう1つ、待つ前に確認すべきことがあります。公式文書は上限をレート型と割り当て型に分けています。レート型は、サービスごとに定められた間隔で自動的にリセットされるため、待てば回復します。割り当て型は、稼働中の資源の個数などを数える上限で、時間では回復しません。資源を解放するか、引き上げを申請する必要があります。上の応答例の CPU 数の上限は割り当て型で、この場合に再試行を繰り返しても意味がありません。`quotaId` と説明文で、どちらの型かを先に見分けてください。

なお、公式のソフトウェア開発キットの多くは、再試行の処理を内部で行います。自分で組む前に、使っている道具が対応していないかを確認してください。

### 原因4：制限の対象が接続元や利用者になっている

`subject` に接続元が入っている場合や、上限の名前に利用者ごとの単位が付いている場合です。プロジェクト全体の上限ではなく、特定の呼び出し元に対する制限です。

この形では、上限の引き上げを申請しても解決しないことが多くあります。複数の実行環境や身元に分散させる、あるいは呼び出しの間隔を空けるなど、呼び出し側の構成を変える必要があります。
...
権限が足りない場合は 403 です。区分の定義には、資源を使い切ったことによる拒否に 403 の区分を使ってはならず、量の超過の区分を使うこと、と明記されています（[GCP の 403 の記事](https://errorlog.jp/posts/gcp_403/)）。ただし、例外が公式に文書化されています。公式のクォータのトラブルシューティングによれば、Compute Engine のクォータ超過は 403 のまま返り、識別子は `QUOTA_EXCEEDED`、レートクォータの場合は `RATE_LIMIT_EXCEEDED` になります。上限の問題が常に 429 で現れるとは限らない、と押さえてください。
...
1. `details` に上限の詳細があるかを見る。あれば推測は不要。無ければ `message` の文言を読む。
2. `apiService` を見る。呼び出したサービスと違えば、調べる先はそちら。
3. `quotaId` と `quotaDimensions` を控える。この2つで上限が一意に決まる。
4. 上限の型を見分ける。レート型は待てば回復、割り当て型は資源の解放か引き上げ。
5. `subject` を見る。プロジェクトなら引き上げが有効、接続元や利用者単位なら呼び出し側の構成を変える。
6. 待ち時間の指示があれば、その時間を守る。無ければ間隔を倍々に伸ばす。公式の指針では最初の間隔は最短30秒。
7. 台数が増えたときに出始めたなら、並列の実行環境が上限を分け合っている。
8. 引き上げの申請は、`quotaId` を添えて行う。名前を伝えられれば手続きが早い。
...
利用者単位の上限の実例として、HashiCorp が公式のサポート記事（[Google API Error 429 Quota exceeded for 'Queries'](https://support.hashicorp.com/hc/en-us/articles/1500009347021-Google-API-Error-429-Quota-exceeded-for-Queries-and-limit-reached-for-Queries-per-minute-per-user)）で扱っている形があります。Terraform の GCP プロバイダで大量の資源を管理していると、実行のたびに全資源の状態を確認しに行くため、「Queries per minute per user」という利用者単位のレート上限に当たる、というものです。応答は古い形式で `details` が無く、指標・上限・サービス・利用者がすべて `message` に書かれていました。示されている解決策は、長寿命の資源を別の作業単位に分離して確認の回数自体を減らす、自動の実行を止める、実行の間隔を空ける、複数の身元に分散する——いずれも呼び出し側の構成を変える方向です。利用者単位のレート上限では引き上げよりも呼び出し方を変える、という原因4の結論と重なります。
...
429 に当たったら、まず `details` を開く。`quotaId` を控える。そして、レート型か割り当て型かを見分ける。待つべきか、減らすべきか、解放すべきかは、そこで決まります。
