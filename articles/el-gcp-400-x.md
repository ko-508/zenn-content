...
名札の貼り替えが実際に起きている記録もあります。開発支援の道具から Gemini の窓口を呼んだ際の報告（[Aider-AI/aider Issue #3817](https://github.com/Aider-AI/aider/issues/3817)）では、返ってきた応答そのものは `status` が `INVALID_ARGUMENT`、`ErrorInfo` の `reason` が `API_KEY_INVALID` の 400 でした。鍵という引数が、それ自体として不正だという通知です。ところが間に挟まった部品はこれを認証のエラーとして包み直し、提供元があなたを認証できない、という文言で表示しています。この表示から 401 を想定して認証の仕組みを調べ始めれば、遠回りです。応答の `status` と `reason` を直接読む、という本記事の順序は、途中の道具が貼り替えた名札に惑わされないための順序でもあります。
